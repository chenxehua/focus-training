  ...
  > {
    return query(
      `SELECT
       ...
       WHERE child_id = ?
         AND DATE(created_at) >= ?
         AND DATE(created_at) < DATE_ADD(?, INTERVAL 1 MONTH)
       GROUP BY DATE(created_at)
       ORDER BY date ASC`,
      [childId, monthStart, monthStart]
    )
  }

  static async getGameBreakdownForMonth(
    childId: number,
    monthStart: string
  ): Promise<Array<{ game_code: string; game_name: string; count: number; avg_score: number }>> {
    return query(
      `SELECT
       ...
       WHERE tr.child_id = ?
         AND DATE(tr.created_at) >= ?
         AND DATE(tr.created_at) < DATE_ADD(?, INTERVAL 1 MONTH)
       GROUP BY g.id
       ORDER BY count DESC`,
      [childId, monthStart, monthStart]
    )
