...
import { adaptQuickSortConfig, getQuickSortConfig, type QuickSortGameConfig } from '@/composables/adapters/quickSortAdapter'
...
const ageAdaptiveConfig = ref<QuickSortGameConfig | null>(null)

// 年龄适配hook
const { config: apiConfig } = useAgeAdaptiveGame('quick_sort', childAgeGroup.value)
...
onMounted(() => {
  ...
  if (isAssessmentMode.value) {
    ageAdaptiveConfig.value = adaptQuickSortConfig(apiConfig.value?.parameters, childAgeGroup.value)
  }
})
...
function handleAssessmentComplete() {
  if (gameResultCallback.value) {
    const resultData = {
      gameCode: 'quick_sort',
      score: resultScore.value,
      accuracy: totalItems.value > 0 ? correctCount.value / totalItems.value : 0,
